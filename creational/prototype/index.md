...
There's one more problem with the direct approach. Since you have to know the object's class to create a duplicate, your code becomes dependent on that class. Additionally, sometimes you only know the interface that the object follows, but not its concrete class.
...
A game character system where new character variants (Knight, Paladin) are produced by cloning a base `warrior` prototype instead of constructing them from scratch.
...
# Clone and customise — no constructor calls needed
